...
<head>
    ...
    <script src="../../js/find_course.js"></script>
...
<content>
    ...
    <div class='item content' id='content'>
      <h1><b>Sửa khóa học</b></h1>
        <?php
        require "../../connect.php";
        $sqlCourse = "SELECT * FROM khoa_hoc";
        $resultCourse = $conn->query($sqlCourse); 
        ?>
        <select class="form-select" aria-label="Default select example" id='value-course'>
          <?php
          for($i=1;$resultCourse->num_rows>=$i;$i++){
            $rowCourse = $resultCourse->fetch_assoc();
            $idCourse = $rowCourse['ma_khoa_hoc'];
